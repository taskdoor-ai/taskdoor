...
def prepare(directory, cases=None):
    ...
    expectations_bytes = (ROOT / "evals/industry-expectations.json").read_bytes()
...
def check(directory):
    ...
    expectations_path = snapshot_path if has_snapshot else ROOT / "evals/industry-expectations.json"
    ...
    expectations = {item["scenarioId"]: item for item in json.loads(expectations_bytes)["expectations"]}
    results = []
...
if __name__ == "__main__":
    ...
    try:
        result = prepare(args.directory, args.cases) if args.command == "prepare" else check(args.directory)
