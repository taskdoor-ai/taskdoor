...
- AgentDoor foundation：核对 Task List 与 Task 详情的关联任务；现有任务身份、状态与人员组件已经覆盖字段呈现。
...
| AgentDoor 现有任务与关联对象行 | 已在 Task List 与 Task 详情使用 | 原生 Button、具名操作与现有焦点模式可直接组合 | 无新依赖；完全受现有 Token 治理 | 最低，且能保持同一 Task 身份外观 | **adopt** |
...
- AgentDoor 适配：复用 `TaskIcon` 还原同一 Task 图标与背景，复用 `TaskStatusBadge` 和 `PersonAvatar`；字号、间距、圆角、触控与颜色只用现有 `--ad-text-*`、`--ad-space-*`、`--ad-radius-*`、`--ad-control-*` 与语义色 Token。
