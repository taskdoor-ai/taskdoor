...
| --- | --- | --- | --- | --- | --- |
| [21st Account Settings](https://21st.dev/community/components/s/account) / [Settings Pages 指南](https://21st.dev/blog/react-settings-page-components) | Account Settings 分类约 56、Glass Account Settings Card 约 277；指南建议按主题拆分、文本显式保存 | 结构建议可靠，但单组件键盘契约与许可链不完整 | 部分候选依赖 framer-motion；21st 单页维护信号有限 | 玻璃态和账号/账单语义不适合 TaskDoor | **adapt structure**：采用“按主题分区、每区独立保存”，拒绝视觉 |
...
| [Base UI Tabs](https://base-ui.com/react/components/tabs) + [Select](https://base-ui.com/react/components/select) | 官方持续维护；项目已直接依赖 `@base-ui/react` | Tabs 提供键盘焦点与 panel 关系；Select 提供 label、typeahead、触控定位 | MIT；无新增依赖 | 只需 TaskDoor Token 适配 | **adopt behavior** |
...
| [shadcn Textarea](https://ui.shadcn.com/docs/components/base/textarea) | 官方提供 Field、Description、Invalid 和按钮组合 | 原生 textarea 键盘与读屏行为直接，错误状态明确 | 项目已有共享 Textarea，无新增依赖；成熟官方实现 | 可完全沿用 TaskDoor Token | **adopt**：单一纯文本责任正文与显式保存 |
...
| [21st Team Access Card](https://21st.dev/%40ruixen.ui/components/team-access-card) | 2025-10-09 发布；覆盖邀请、角色和删除 | 提供成员管理全链路参考，但卡片 / Drawer 结构不符合大型设置 Dialog | 依赖 shadcn / Framer Motion；社区维护信号弱于官方 primitive | 改造成 TaskDoor 设置页的成本可控 | **adapt workflow / reject code**：采用动作集合，拒绝卡片与动效视觉 |
...
- 采用的视觉结构：改造 21st Settings 的“左侧类别 / 右侧详情”大型设置壳层和主题分区；头像菜单继续复用现有 TaskDoor 入口。
...
- 为什么不需要自研 / 为什么必须自研：Select、Dialog 行为复用成熟 primitive；横向分组采用 CSS Grid 和现有 Claim 组合，无需新增 Card 或 Grid 依赖。“拟写入文本、具体证据与人工确认”是 TaskDoor 领域语义，继续由产品组件表达。r1 曾创建的未引用 Tabs 已移除。
