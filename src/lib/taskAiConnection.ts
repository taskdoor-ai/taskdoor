...
export function buildTaskAiConnectionRequest(input: TaskAiConnectionInput): AiConnectionRequest {
  const { taskId, task, currentUser, pendingOwnerId, parentTask } = input;
  const context: AiConnectionRequest["context"] = [
    ...
    { label: "参与人", value: task.participants.length ? task.participants.map(id => {
      const status = task.participantInvitationStatus?.[id];
      return `${id} · ${status === "accepted" ? "已接受" : status === "pending" ? "待接受，尚未生效" : "接受状态未提供"}`;
    }).join("\n") : "任务传入快照未列出参与人" },
    { label: "截止日期", value: providedValue(input.due ?? task.due) },
  ];
  if (pendingOwnerId?.trim()) context.push({ label: "负责人提议", value: `${pendingOwnerId} · 待接受，尚未生效，不替换正式负责人` });
  ...
  const taskFields = [
    "Task ID", "名称", "目标", "状态", "期限", "人员",
    joinedValues(task.completionCriteria) ? "完成标准" : "",
    joinedValues(task.executionTips) ? "执行建议" : "",
    input.tags?.length ? "标签" : "",
  ].filter(Boolean);
  return {
    title: "连接 AI",
    ...
    contextPreview: {
      ...
      items: [
        { id: "task", label: "当前任务", title: task.title || "未命名任务", detail: taskFields.join("、") },
        ...(parentTask ? [{ id: "parent-task", label: "主任务", title: parentTask.title, detail: "任务 Task ID、名称、目标、状态" }] : []),
      ],
    },
