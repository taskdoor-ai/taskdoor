...
/** Serializes only the current user's formally owned task-list projection. */
  ...
  return {
    ...
    contextPreview: {
      ...
      items: [{
        ...
        label: "我的任务列表",
        title: "我的任务列表",
        detail: `共 ${model.ownedTasks.length} 项正式负责的任务 · 每项包含 Task ID、名称、状态 · 数据快照 ${model.asOf || "时间待核对"}`,
      }],
    },
    workObject: {
      kind: "任务列表",
      title: "我的任务列表",
      meta: `共 ${model.ownedTasks.length} 项正式负责的任务 · 数据快照 ${model.asOf || "时间待核对"}`,
    },
    context: [
      {
        label: "来源与范围",
        value: "仅带入当前用户在当前可见范围内正式负责的完整任务列表，每项只含 Task ID、名称和真实状态，不受左侧搜索、状态或标签筛选影响；不带入其他成员任务、待接受负责人提议、排序建议、讨论、文件正文或任务详情。列表快照不代表生产权限核验，也不会扩大权限。",
      },
      ...tasks,
    ],
  };
