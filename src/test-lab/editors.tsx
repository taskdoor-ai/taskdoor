...
import { skillIds, taskStatuses, type Json, type LabAssertion, type LabCase, type LabEvidence, type LabEvent, type LabMember, type LabSkill, type LabTask, type LabTeam } from "./types";
...
const resourceNames:Record<LabEvidence["kind"],string>={文件:"新文件",讨论:"任务讨论",交付:"交付记录",确认:"确认记录"};
...
function TaskFields({team,task,onTaskChange,onEvidenceChange,showResources=false,requireCreator=false}:{team:LabTeam;task:LabTask;onTaskChange:(patch:Partial<LabTask>)=>void;onEvidenceChange?:(items:LabEvidence[])=>void;showResources?:boolean;requireCreator?:boolean}){
  const taskEvidence=team.evidence.filter((item)=>item.taskId===task.id);
  const changeEvidence=(id:string,patch:Partial<LabEvidence>)=>onEvidenceChange?.(team.evidence.map((item)=>item.id===id?{...item,...patch,version:item.version+1}:item));
  ...
  return <>
    ...
    <div className="lab-form-grid"><Field label="任务状态"><select value={task.status} onChange={(event)=>onTaskChange({status:event.target.value as LabTask["status"]})}>{taskStatuses.map((status)=><option key={status}>{status}</option>)}</select></Field>
      ...
      <Field label="负责人"><select value={task.ownerId||""} onChange={(event)=>onTaskChange({ownerId:event.target.value||null})}><option value="">未指派</option>{team.members.map((member)=><option key={member.id} value={member.id}>{member.name}</option>)}</select></Field>
    ...
    <Choices label="参与人" items={team.members} value={task.participantIds} onChange={(participantIds)=>onTaskChange({participantIds})}/><Choices label="前置任务" items={team.tasks.filter((item)=>item.id!==task.id).map((item)=>({id:item.id,name:item.title}))} value={task.dependsOnTaskIds} onChange={(dependsOnTaskIds)=>onTaskChange({dependsOnTaskIds})}/><Lines label="完成标准" value={task.acceptanceCriteria} onChange={(acceptanceCriteria)=>onTaskChange({acceptanceCriteria})}/><Lines label="执行建议" value={task.executionTips} onChange={(executionTips)=>onTaskChange({executionTips})}/><Lines label="标签" value={task.tags} onChange={(tags)=>onTaskChange({tags})}/>
    {showResources&&<div className="lab-task-resources"><ResourceList title="任务文件" kinds={["文件"]}/><ResourceList title="任务讨论" kinds={["讨论"]}/><ResourceList title="交付与确认记录" kinds={["交付","确认"]}/><p className="lab-muted">文件用于构造测试上下文，仅保存名称与正文/摘要，不上传真实二进制文件。</p></div>}
  </>;
}
...
export function CaseEditorFields({ value, teams, skills, onChange }: { value: LabCase; teams: LabTeam[]; skills: LabSkill[]; onChange: (value: LabCase) => void }) {
  ...
  return <>
    ...
    <Text label="测试目的与说明" multiline value={value.description} onChange={(description) => set({ description })} />
    ...
    <section className="lab-editor-section"><header><h3>执行步骤</h3><button type="button" onClick={() => set({ steps: [...value.steps, newStep()] })}>＋ 添加步骤</button></header>
      {value.steps.map((step, index) => { const update = (patch: Partial<typeof step>) => set({ steps: value.steps.map((item) => item.id === step.id ? { ...item, ...patch } : item) }); return <fieldset key={step.id} className="lab-form-card"><legend>步骤 {index + 1}</legend>
        <div className="lab-form-grid"><Field label="测试技能"><select value={step.skillId} onChange={(event) => update({ skillId: event.target.value as typeof step.skillId })}>{skillOptions.map((skill) => <option key={skill.id} value={skill.id}>{skill.title}</option>)}</select></Field>
          <Field label="目标任务"><select value={step.taskId || ""} onChange={(event) => update({ taskId: event.target.value || null })}><option value="">全部可见任务 / 规划新任务</option>{team?.tasks.map((task) => <option key={task.id} value={task.id}>{task.title}</option>)}</select></Field></div>
