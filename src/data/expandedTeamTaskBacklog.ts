...
export const platformAdditionNodes: TaskNode[] = [
  ...
  platformTask({ id: "platform-monthly-privileged-review", name: "月度紧急生产权限复核", ownerId: "宋衡", participantIds: ["叶宁", "周岚"], status: "进行中", goal: "核对本月临时提权、过期授权和紧急访问是否已关闭并留存理由。", completionCriteria: ["全部临时权限有到期或续期结论", "紧急访问与对应事件记录关联", "孤立账号和异常角色差异已处置"], executionTips: ["与季度全量访问复核区分，本任务只检查临时和紧急权限"], dueAt: "9 月 4 日", plannedStartOn: "2026-09-01", plannedEndOn: "2026-09-04", labels: ["安全门禁"], iconName: "file-check", iconTone: "red", minutes: 240 }),
...
export const supplyOperationsAdditionNodes: TaskNode[] = [
  ...
  supplyTask({ id: "supply-winter-demand-scenarios", name: "形成冬季促销三档需求情景", parentTaskId: "supply-winter-plan-track", ownerId: "陈琛", participantIds: ["周岚", "罗骁"], status: "进行中", goal: "以基准、上行和延迟促销三种情景表达周度需求与承诺差异。", completionCriteria: ["销售输入、历史偏差和渠道拆分可追溯", "每档情景包含触发条件与更新时间", "异常大单不被均摊到普通需求"], executionTips: ["历史销量只作为参考，不自动视为本季承诺"], dueAt: "9 月 12 日", plannedStartOn: "2026-09-01", plannedEndOn: "2026-09-12", labels: ["物料齐套"], iconName: "chart", iconTone: "blue", minutes: 480 }),
  ...
  supplyTask({ id: "supply-monthly-sop-floor-audit", name: "月度现场 SOP 版本抽查", ownerId: "唐静", participantIds: ["赵妍", "罗骁", "贺青"], status: "进行中", goal: "确认关键工位实际使用的电子和纸质 SOP 与受控版本一致。", completionCriteria: ["关键工位全部完成版本核对", "私印、手写修改和过期副本已回收", "版本差异对应人员完成补训"], executionTips: ["抽查覆盖白班与夜班，不只检查示范工位"], dueAt: "9 月 7 日", plannedStartOn: "2026-09-01", plannedEndOn: "2026-09-07", labels: ["SOP", "人员培训"], iconName: "clipboard-check", iconTone: "purple", minutes: 300 }),
