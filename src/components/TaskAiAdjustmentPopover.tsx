...
export function TaskAiAdjustButton({ label = "AI 帮你改", onClick, disabled = false }: { label?: string; onClick: (anchor: HTMLElement) => void; disabled?: boolean }) {
  ...
  return <Button aria-haspopup="dialog" aria-label={label === "AI 帮你改" ? d("adjustWithAi") : label} className="task-ai-adjust-trigger" disabled={disabled} onClick={event => onClick(event.currentTarget)} size="sm" type="button" variant="ai"><Sparkles aria-hidden="true" size={14} />{d('adjustWithAi')}</Button>;
}
...
export function TaskAiAdjustmentPopover({ context, scope: requestedScope, compactCreation = false, history, onScopeChange, onProgressChange, onApply, open, onOpenChange, anchor, draftSession }: Props) {
  ...
  const scopeLabel = scope.kind === "subtasks" ? ui("子任务安排") : scope.kind === "subtask" ? ui("单个子任务") : d('information');
  ...
  const availableScopes: Array<{ scope: TaskAiAdjustmentScope; label: string; disabled?: boolean }> = [
    { scope: { kind: "task" }, label: d('information') },
    ...(context.canAddSubtasks || context.subtasks.length ? [{ scope: { kind: "subtasks" } as const, label: "子任务安排" }] : []),
    ...context.subtasks.map((task, index) => ({ scope: { kind: "subtask", taskId: task.id } as const, label: ui("子任务 {0}：{1}", {0: index + 1, 1: task.title || "未命名子任务"}) })),
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      finishProgress("stopped", ui("预览已中止，输入已保留。"), draftKey, true);
    };
  }, [open, draftKey]);

  useEffect(() => {
    if (stale && !applying.current) finishProgress("completed", ui("内容已变化，当前预览已过期。请重新预览，输入已保留。"), draftKey, false, { applicationStatus: "expired" });
  ...
  const invalidatePreview = () => {
    ...
    previewController.current = null;
    setPreviewing(null);
    finishProgress("stopped", ui("已停止预览，输入已保留。"), draftKey, true);
  };
  const stopPreview = () => {
    invalidatePreview();
    setNotice(ui("已停止预览，输入已保留。"));
  ...
  const changeInstruction = (value: string) => {
    ...
    if (proposal) finishProgress("completed", ui("输入已更新，原候选未应用；可重新预览。"), draftKey, false, { applicationStatus: "not_applied" });
  ...
  const preview = async () => {
    ...
    try {
      ...
      if (latestPreview.current.contextSignature !== contextSignature) {
        setError(ui("任务内容或成员信息已变化，请重新预览。输入已保留。"));
        finishProgress("failed", ui("任务内容或成员信息已变化，请重新预览。输入已保留。"));
        return;
      }
  ...
  const composer = <form aria-busy={saving || isPreviewing} id={composerId} onSubmit={event => { event.preventDefault(); void preview(); }}>
    {compact ?
      <AnimatedAgentChatInput
        ...
        ariaLabel={ui("继续告诉 TaskDoor 怎样调整任务方案")}
        autoFocus={false}
        clearOnSend={false}
        disabled={saving}
        hint={ui("Enter 发送 · Shift + Enter 换行")}
        inputRef={inputRef}
        onChange={changeInstruction}
        onSend={() => { void preview(); }}
        onStop={stopPreview}
        placeholder={ui("继续告诉 TaskDoor 你想怎样调整…")}
        sendLabel={ui("发送修改要求")}
        status={isPreviewing ? "analyzing" : "ready"}
        value={instruction}
      /> : <>
      <label className="sr-only" htmlFor={`${composerId}-input`}>{ui("你想怎样调整？")}</label>
      <Textarea aria-invalid={Boolean(error)} disabled={saving || isPreviewing} id={`${composerId}-input`} onChange={event => changeInstruction(event.target.value)} onKeyDown={event => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter" && !event.nativeEvent.isComposing) { event.preventDefault(); void preview(); } }} placeholder={scope.kind === "subtasks" ? ui("例如：子任务「任务名称」：负责人改为林洁") : ui("例如：增加完成标准：交付结果已与相关成员核对")} ref={inputRef} rows={3} value={instruction} />
      <div className="task-ai-adjust-composer-actions">{isPreviewing ? <TaskAiWorking cancelLabel={ui("停止预览调整")} detail={ui("仅调整{0}，先预览，确认后应用", {0: scopeLabel})} label={previewing?.step === 1 ? ui("整理修改预览") : ui("理解调整要求")} onCancel={stopPreview} /> : <><span><Sparkles aria-hidden="true" size={14} />{ui("先预览，确认后应用")}</span>{!proposal && <Button aria-label={ui("预览调整")} className="task-ai-adjust-send" disabled={saving || !instruction.trim()} size="icon" title={ui("预览调整 · ⌘ / Ctrl + Enter")} type="submit"><ArrowUp aria-hidden="true" size={19} /></Button>}</>}</div>
    </>}
  </form>;
  const proposalActionButtons = <>
    <Button disabled={saving} onClick={cancel} type="button" variant="ghost">{d('cancel')}</Button>
    {proposal && !stale ? <Button className="task-ai-adjust-primary" disabled={saving} onClick={() => void apply()} type="button">{saving ? <LoaderCircle aria-hidden="true" className="task-ai-adjust-spinner" size={15} /> : <Check aria-hidden="true" size={15} />}{saving ? ui("正在保存") : context.mode === "draft" ? ui("应用到草稿") : d('saveChanges')}</Button> : <Button className="task-ai-adjust-primary" disabled={saving || !instruction.trim()} form={composerId} type="submit">{stale ? ui("重新预览") : ui("预览调整")}<ArrowRight aria-hidden="true" size={15} /></Button>}
  </>;
  ...
  return <Popover modal={false} onOpenChange={(value, details) => {
    ...
    <PopoverContent align="end" anchor={anchor} aria-label={ui("AI 帮你改：{0}", {0: target?.title || ui("未命名任务")})} className={`task-ai-adjust-popover${compact ? " is-compact" : ""}`} collisionPadding={8} finalFocus={finalFocusTarget} initialFocus={inputRef} onKeyDownCapture={handleTabBoundary} positionerClassName={compact ? "task-ai-adjust-fixed-positioner" : undefined} positionMethod="fixed" sideOffset={10}>
      {compact ? <header className="task-ai-adjust-conversation-header">
        <div><span><Sparkles aria-hidden="true" size={15} /><strong>{ui("本次创建对话")}</strong></span><small>{history?.length ?? 0}{ui("轮")}</small></div>
        <Button aria-label={ui("收起 AI 帮你改")} className="task-ai-adjust-close" disabled={saving} onClick={dismiss} size="icon-sm" title={ui("收起，保留输入")} type="button" variant="ghost"><X aria-hidden="true" size={16} /></Button>
      </header> : <header className="task-ai-adjust-header">
        {onScopeChange ? <div className="task-ai-adjust-scope-picker">
          <label htmlFor={`${composerId}-scope`}><ScopeIcon aria-hidden="true" size={14} />{ui("调整范围")}</label>
          <div className="task-ai-adjust-scope-control">
            <select disabled={saving || isPreviewing} id={`${composerId}-scope`} onChange={event => changeScope(event.target.value)} ref={scopeSelectRef} value={draftKey}>
              {scopeOptions.map(option => <option disabled={option.disabled} key={option.key} value={option.key}>{ui(option.label)}</option>)}
            </select>
            <ChevronDown aria-hidden="true" size={14} />
          </div>
        </div> : <span aria-label={ui("调整范围")} className="task-ai-adjust-scope"><ScopeIcon aria-hidden="true" size={14} /><span>{ui("调整：")}{scopeLabel}</span><strong title={target?.title || ui("未命名任务")}>{target?.title || ui("未命名任务")}</strong></span>}
        <Button aria-label={ui("收起 AI 帮你改")} disabled={saving} onClick={dismiss} size="icon-sm" title={ui("收起，保留输入")} type="button" variant="ghost"><X aria-hidden="true" size={16} /></Button>
      </header>}
      <div aria-label={compact ? ui("本次创建对话和修改预览") : undefined} className="task-ai-adjust-body" onScroll={compact ? event => {
        ...
        {!compact && composer}
        {!compact && error && <p className="task-ai-adjust-error" role="alert">{ui(error)}</p>}
        {!compact && notice && <p className="task-ai-adjust-notice" role="status">{ui(notice)}</p>}
        ...
        {!isPreviewing && proposal && <section aria-label={ui("待应用的调整")} className="task-ai-adjust-preview" ref={previewRef} tabIndex={-1}>
          <header><span><Check aria-hidden="true" size={15} />{ui("本次修改")}</span><small>{additions.length ? ui("新增 {0} 个子任务", {0: additions.length}) : ui("{0} 处变化", {0: fieldChanges.length})}</small></header>
          {additions.map(task => <article aria-label={ui("新增子任务：{0}", {0: task.title})} className="task-ai-adjust-addition" key={task.id}><h3><GitBranch aria-hidden="true" size={15} />{task.title}</h3><dl>{proposal.changes.filter(change => change.taskId === task.id && change.label !== "新增子任务").map(change => <div key={change.label}><dt>{change.label}</dt><dd>{change.after || d('notSet')}</dd></div>)}</dl></article>)}
          <ul>{fieldChanges.map((change, index) => <li key={`${change.taskId}:${change.label}:${index}`}>
            ...
            <div className="task-ai-adjust-diff"><div className="task-ai-adjust-before"><small>{ui("修改前")}</small><p>{change.before || d('notSet')}</p></div><ArrowRight aria-hidden="true" size={14} /><div className="task-ai-adjust-after"><small>{ui("修改后")}</small><p>{change.after || d('notSet')}</p></div></div>
          </li>)}</ul>
          {!compact && <p className="task-ai-adjust-summary">{proposal.summary}</p>}
          {stale && <p className="task-ai-adjust-error" role="alert">{ui("内容已变化，当前预览已过期。请重新预览，避免覆盖最新修改。")}</p>}
          {compact && <footer className="task-ai-adjust-preview-actions"><span>{stale ? ui("当前候选已过期") : ui("当前候选待应用")}</span><div>{proposalActionButtons}</div></footer>}
        </section>}
      </div>
      {compact && <div className="task-ai-adjust-conversation-composer">
        {historyHasNew && <button className="task-ai-adjust-new-reply" onClick={scrollHistoryToEnd} type="button">{ui("查看最新进度")}</button>}
        {composer}
      </div>}
      {!compact && !isPreviewing && proposal && <footer className="task-ai-adjust-footer">
        <span>{context.mode === "draft" ? ui("应用后仍需确认创建") : ui("保存后记录到任务活动")}</span>
        {proposalActionButtons}
      </footer>}
      {!compact && !isPreviewing && !proposal && instruction && <div className="task-ai-adjust-draft-actions"><span>{ui("收起后可继续编辑")}</span><Button disabled={saving} onClick={cancel} size="sm" type="button" variant="ghost">{d('cancel')}</Button></div>}
    </PopoverContent>
  </Popover>;
}
