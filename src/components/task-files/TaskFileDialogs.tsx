...
export function TaskFileDialogs({ dialog, nodes, onClose, onCreate, onDelete, onIcon, onMove, onRename }: {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="task-file-action-dialog"><DialogHeader><DialogTitle>{ui(title)}</DialogTitle><DialogDescription>{dialog.type === "create" ? ui("创建位置：{0}", {0: dialog.parentId ? getNodePath(nodes, dialog.parentId).map((item) => item.name).join(" / ") : ui("任务文件根目录")}) : node?.name}</DialogDescription></DialogHeader>
    {(dialog.type === "create" || dialog.type === "rename") && <label className="task-file-dialog-field"><span>{ui("名称")}</span><input autoFocus onChange={(event) => setName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") run(() => dialog.type === "create" ? onCreate(dialog.parentId, name) : onRename(node!.id, name)); }} value={name} /></label>}
    {dialog.type === "move" && <div className="task-file-dialog-targets"><button className={target === null ? "active" : ""} onClick={() => setTarget(null)} type="button"><Folder size={15} />{ui("任务文件根目录")}</button>{folders.map((folder) => <button className={target === folder.id ? "active" : ""} key={folder.id} onClick={() => setTarget(folder.id)} type="button" style={{ paddingLeft: 14 + (getNodePath(nodes, folder.id).length - 1) * 14 }}><Folder size={15} />{folder.name}</button>)}</div>}
    {dialog.type === "icon" && <div className="task-file-icon-grid">{iconOptions.map(({ name: iconName, label, Icon }) => <button key={iconName} onClick={() => run(() => onIcon(node!.id, iconName))} title={ui(label)} type="button"><Icon size={18} /><span>{ui(label)}</span></button>)}</div>}
    {dialog.type === "delete" && node?.kind === "folder" && children > 0 && <div className="task-file-delete-policies"><p>{ui("此文件夹包含")}{children}{ui("个直接子项，请选择处理方式。")}</p><label><input checked={policy === "move-contents"} name="folder-delete-policy" onChange={() => setPolicy("move-contents")} type="radio" />{ui("将内容移动到上一级后删除")}</label><label><input checked={policy === "archive"} name="folder-delete-policy" onChange={() => setPolicy("archive")} type="radio" />{ui("将整个文件夹及内容归档")}</label></div>}
    {dialog.type === "delete" && (node?.kind === "file" || children === 0) && <p className="task-file-delete-copy">{ui("删除后可通过页面底部提示立即撤销。")}</p>}
    {error && <p className="task-file-dialog-error" role="alert">{ui(error)}</p>}
    {dialog.type !== "icon" && <DialogFooter><Button onClick={onClose} variant="outline">{ui("取消")}</Button><Button disabled={(dialog.type === "create" || dialog.type === "rename") ? !name.trim() : false} onClick={() => run(() => { if (dialog.type === "create") onCreate(dialog.parentId, name); else if (dialog.type === "rename") onRename(node!.id, name); else if (dialog.type === "move") onMove(node!.id, target); else onDelete(node!.id, node?.kind === "folder" ? policy : undefined); })} variant={dialog.type === "delete" ? "destructive" : "default"}>{dialog.type === "delete" ? ui("确认删除") : dialog.type === "move" ? ui("移动") : ui("保存")}</Button></DialogFooter>}
