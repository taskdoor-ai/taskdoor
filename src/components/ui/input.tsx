...
function Textarea({ className, variant = "default", ...props }: React.ComponentProps<"textarea"> & { variant?: "default" | "document" | "responsibility" }) {
  return <textarea data-slot="textarea" className={cn("min-h-[calc(var(--ad-control-height-lg)*2.5)] w-full resize-y rounded-(--ad-radius-control) border border-(--ad-border) bg-(--ad-surface) px-(--ad-space-3) py-(--ad-space-2) font-sans text-(length:--ad-text-body-sm) leading-6 text-(--ad-ink) outline-none transition-[border-color] placeholder:text-(--ad-ink-tertiary) focus-visible:border-(--ad-focus) focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:bg-(--ad-surface-subtle) disabled:text-(--ad-ink-tertiary)", variant === "document" && "min-h-[calc(var(--ad-control-height-lg)*5)] leading-7", variant === "responsibility" && "min-h-(--ad-control-height-lg) resize-none", className)} {...props} />
}
