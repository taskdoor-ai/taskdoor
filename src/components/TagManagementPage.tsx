...
export function TagManagementPage({ embedded = false, onBack, onChange, tags }: Props) {
  ...
  return <article className="tag-management-page">
    {!embedded && <nav aria-label="当前位置" className="tag-management-breadcrumb"><button onClick={onBack} type="button">任务</button><ChevronRight aria-hidden="true" /><strong aria-current="page">我的标签</strong></nav>}
    <header className="tag-management-heading">
      <div className="tag-management-heading-copy"><div className="tag-management-title">{embedded ? <DialogTitle render={<h1 />}>我的标签</DialogTitle> : <h1>我的标签</h1>}<span>{tags.length} 个标签</span></div><p>维护你添加任务标签时可使用的个人标签。</p></div>
    ...
    {tags.length > 0 ? <section aria-label="个人标签列表" className="tag-list-panel">
    ...
    {error && !editor && <p className="tag-editor-field-error" role="alert">{error}</p>}
    <Dialog onOpenChange={(open) => { if (!open) { setEditor(null); setError(""); } }} open={Boolean(editor)}><DialogContent className="tag-editor-dialog"><DialogHeader className="tag-dialog-heading"><span className="tag-dialog-mark"><Tag aria-hidden="true" /></span><div><DialogTitle>{editor?.id ? "编辑标签" : "新建标签"}</DialogTitle><DialogDescription>{editor?.id ? "调整个人标签的名称和识别样式，已保存任务上的标签保持不变。" : "创建一个可直接用于任务的个人标签。"}</DialogDescription></div></DialogHeader>{error && <p className="tag-editor-field-error" role="alert">{error}</p>}{editor && previewTag && <div className="tag-editor-body">
