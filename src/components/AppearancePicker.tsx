...
type Props<Icon extends string, Color extends string> = {
  ...
  colorOptions: Array<{ value: Color; label: string; color: TagColorName | TaskIconTone }>;
...
export function AppearancePicker<Icon extends string, Color extends string>({ color, icon, iconOptions, colorOptions, onColorChange, onIconChange, preview, previewNote, showHints = true, iconColumns = 6, colorColumns = 5 }: Props<Icon, Color>) {
  ...
  return <div className="tag-appearance-picker">
    <div className="tag-editor-preview-stage" data-color={selectedColor?.color ?? color}>
      <span className="tag-editor-preview-label">{u('livePreview')}</span>
      {preview}
      {previewNote && <span className="tag-editor-preview-note">{previewNote}</span>}
    </div>
    <fieldset className="tag-editor-section">
      <legend><span>{u('icon')}</span>{showHints && <small>{u('iconHint')}</small>}</legend>
    ...
    <fieldset className="tag-editor-section">
      <legend><span>{u('background')}</span>{showHints && <small>{selectedColor?.label}</small>}</legend>
