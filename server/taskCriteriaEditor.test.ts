...
test("完成标准默认完整可读，编辑表单按需打开", async () => {
  ...
  assert.match(html, /aria-label="Edit 测试子任务: Acceptance criteria"/);
...
test("详情头部完成标准直接逐行编辑，并与创建页共用添加一行交互", async () => {
  ...
  assert.match(empty, /aria-label="Add 收尾任务: Acceptance criteria"/);
  assert.match(empty, />Add<\/button>/);
  ...
  assert.match(existing, /aria-label="Add Current task: Acceptance criteria"/);
  assert.match(existing, /aria-label="Remove Current task: Acceptance criteria 3"/);
