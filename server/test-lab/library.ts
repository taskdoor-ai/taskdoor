...
export function importedLibrary(){
  ...
  for(const family of ['industry','edge','complex','realistic']){
    ...
    for(const scenario of fixtures.scenarios){
      ...
      const team:LabTeam={id,name:`${originalTeam.name} · ${scenario.id}`,industry:scenario.industry,description:note,archived:false,members,tasks,evidence};
      ...
      cases.push({id,name:`${scenario.id} · ${scenario.category}`,category:'历史资料待复核',teamId:id,actorId:memberIds.has(input.currentUserId)?input.currentUserId:members[0].id,description:team.description,archived:false,enabled:false,version:1,
  ...
  for(const episode of read('multiturn-scenarios.json').episodes){
    ...
    cases.push({...structuredClone(source),id:`import-multiturn-${episode.id}`,name:`多轮 ${episode.id} · ${episode.name}`,description:'历史多轮迁移待复核：当前仅保留对话步骤，不自动模拟旧版草稿提交、响应丢失或权限撤回。必须补齐对应情节后才可作为回归测试。',
...
export function importLibrary(store:LabStore){
  ...
  return {state:cases.length||teams.length?store.save(current.revision,[...current.teams,...teams],[...current.cases,...cases]):current,imported:cases.length};
}
