...
import type { LabRun, LabStepResult } from '../../src/test-lab/types.ts';
import type { LabStore } from './store.ts';
import { buildModelInput, applyEvents } from './context.ts';
import { callModel, ModelResponseError, type ModelOptions, type ModelCall } from './model.ts';
import { hash, loadSkill } from './skills.ts';
...
export function createRunner(store:LabStore,options:ModelOptions,modelCall:ModelCall=callModel){
  const queue:string[]=[];
  const snapshots=new Map<string,ReturnType<typeof loadSkill>[]>();
  ...
  async function execute(id:string){
    ...
    try{
      for(const [index,step] of run.caseSnapshot.steps.entries()){
        ...
        const result:LabStepResult={stepId:step.id,skillId:step.skillId,input:null,output:null,rawOutput:'',error:null,assertions:[],structure:'unknown',structureErrors:[],durationMs:0,usage:{inputTokens:null,outputTokens:null,totalTokens:null},skillHash:skill.hash,promptHash:'',skillSnapshot:skill.snapshot,before:structuredClone(sandbox),after:structuredClone(sandbox)};
        try{
          ...
          const response=await modelCall(options,skill.instructions,input,controller.signal);result.rawOutput=response.rawOutput;result.usage=response.usage;
  ...
  return {
    enqueue(caseIds:string[],requestId:string){
      ...
      if(existing.length){if(JSON.stringify(existing.map(r=>r.caseId))!==JSON.stringify(caseIds))throw new Error('同一请求标识不能用于不同用例');return existing;}
      ...
      const cases=caseIds.map(id=>{const c=state.cases.find(c=>c.id===id);if(!c||c.archived||!c.enabled)throw new Error('用例不存在、已归档或待审核启用');const team=state.teams.find(t=>t.id===c.teamId);if(!team||team.archived)throw new Error('用例团队不存在或已归档');return {c,team};});
      if(cases.reduce((n,{c})=>n+c.steps.length,0)>20)throw new Error('一批最多 20 个步骤，避免意外 API 用量');
      const frozen=cases.map(({c})=>c.steps.map(s=>loadSkill(s.skillId)));
      const batchId=randomUUID();const runs:LabRun[]=cases.map(({c,team})=>({id:randomUUID(),batchId,requestId,caseId:c.id,caseName:c.name,actorId:c.actorId,teamId:c.teamId,status:'queued',createdAt:new Date().toISOString(),startedAt:null,finishedAt:null,model:options.model,endpoint:options.endpoint,caseSnapshot:structuredClone(c),teamSnapshot:structuredClone(team),steps:[],error:null,review:null}));
